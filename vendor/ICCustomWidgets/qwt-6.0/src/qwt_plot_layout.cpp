    ...
    QRectF &canvasRect, QRectF scaleRect[QwtPlot::axisCnt] ) const
{
    ...
    for ( int axis = 0; axis < QwtPlot::axisCnt; axis++ )
    {
        ...
        if ( axis == QwtPlot::xTop || axis == QwtPlot::xBottom )
        {
            ...
            {
                const qreal dx = leftOffset + leftScaleRect.width();
            ...
            {
                const qreal dx = rightOffset + rightScaleRect.width();
        ...
        else // QwtPlot::yLeft, QwtPlot::yRight
        {
            ...
            {
                const qreal dy = bottomOffset + bottomScaleRect.height();
            ...
            {
                const qreal dy = topOffset + topScaleRect.height();
                if ( d_data->alignCanvasToScales && dy < 0 )
                {
                    /*
                      The axis needs more space than the height
